...
public class PlayerController : MonoBehaviour
{
    ...
    {        
        ...
        for (int i = finalPath.Count - 1; i > 0; i--)
        {
            ...
            s.Append(transform.DOMove(finalPath[i].GetComponent<Walkable>().GetWalkPoint(), speed * time).SetEase(Ease.Linear));
    ...
    public void RayCastDown()
    {
        ...
        if (Physics.Raycast(playerRay, out playerHit))
        {
            if (playerHit.transform.GetComponent<Walkable>() != null)
            {
                ...
                else if (playerHit.transform.GetComponent<Walkable>().visited && playerHit.transform.GetComponent<MeshRenderer>().material.color == Color.magenta)
                    playerHit.transform.GetComponent<MeshRenderer>().material.color = Color.gray;
